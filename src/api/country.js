...
import { Country, Info, CommonMimeType } from '@qubit-ltd/common-model';
...
class CountryApi {
  ...
  list(pageRequest = {}, criteria = {}, sortRequest = {}, showLoading = true) {
    ...
    checkObjectArgument('criteria', criteria, COUNTRY_CRITERIA_DEFINITIONS);
  ...
  update(country, showLoading = true) {
    checkArgumentType('country', country, [Country, Object]);
    checkIdArgumentType(country.id, 'country.id');
    checkArgumentType('showLoading', showLoading, Boolean);
  ...
  exportXml(criteria = {}, sortRequest = {}, autoDownload = true, showLoading = true) {
    ...
    return http.download('/country/export/xml', params, mimeType, autoDownload).then((result) => {
      logger.info('Successfully export the Country to XML:', result);
      return result;
    });
  }
}
