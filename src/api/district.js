...
//
////////////////////////////////////////////////////////////////////////////////
import { District, Info } from '@qubit-ltd/common-model';
...
class DistrictApi {
  ...
  list(pageRequest = {}, criteria = {}, sortRequest = {}, showLoading = true) {
    return listImpl(this, '/district', pageRequest, criteria, sortRequest, showLoading);
  }
  ...
  listInfo(pageRequest = {}, criteria = {}, sortRequest = {}, showLoading = true) {
    return listInfoImpl(this, '/district/info', pageRequest, criteria, sortRequest, showLoading);
  }

  /**
   * 根据ID，获取指定的`District`对象。
   *
  ...
  get(id, showLoading = true) {
    return getImpl(this, '/district/{id}', id, showLoading);
  }

  /**
   * 根据代码，获取指定的`District`对象。
   *
   * @param {string} code
   *     `District`对象的代码。
   * @param {boolean} showLoading
  ...
  getByCode(code, showLoading = true) {
    return getByKeyImpl(this, '/district/code/{code}', 'code', code, showLoading);
  }

  /**
   * 根据ID，获取指定的`District`对象的基本信息。
   *
   * @param {string|number|bigint} id
   *     `District`对象的ID。
   * @param {boolean} showLoading
  ...
  getInfo(id, showLoading = true) {
    return getInfoImpl(this, '/district/{id}/info', id, showLoading);
  }

  /**
   * 根据代码，获取指定的`District`对象的基本信息。
   *
   * @param {string} code
   *     `District`对象的代码。
   * @param {boolean} showLoading
  ...
  getInfoByCode(code, showLoading = true) {
    return getInfoByKeyImpl(this, '/district/code/{code}/info', 'code', code, showLoading);
  }

  /**
   * 添加一个`District`对象。
   *
   * @param {District|object} entity
   *     要添加的`District`对象。
   * @param {boolean} showLoading
   *     是否显示加载提示。
   * @return {Promise<District|ErrorInfo>}
  ...
  add(entity, showLoading = true) {
    return addImpl(this, '/district', entity, showLoading);
  }
  ...
  update(entity, showLoading = true) {
    return updateImpl(this, '/district/{id}', entity, showLoading);
  }

  /**
   * 根据代码，更新一个`District`对象。
   *
   * @param {District} entity
   *     要更新的`District`对象的数据，根据其代码确定要更新的对象。
   * @param {boolean} showLoading
   *     是否显示加载提示。
   * @return {Promise<District|ErrorInfo>}
   *     此HTTP请求的`Promise`对象。若操作成功，则解析成功并返回更新后的`District`对象；
  ...
  updateByCode(entity, showLoading = true) {
    return updateByKeyImpl(this, '/district/code/{code}', 'code', entity, showLoading);
  }
  ...
  delete(id, showLoading = true) {
    return deleteImpl(this, '/district/{id}', id, showLoading);
  }

  /**
   * 根据代码，标记删除一个`District`对象。
   *
   * @param {string} code
   *     要标记删除的`District`对象的代码。
   * @param {boolean} showLoading
   *     是否显示加载提示。
   * @return {Promise<string|ErrorInfo>}
   *     此HTTP请求的`Promise`对象。若操作成功，则解析成功并返回数据被标记删除的UTC时间戳，
  ...
  deleteByCode(code, showLoading = true) {
    return deleteByKeyImpl(this, '/district/code/{code}', 'code', code, showLoading);
  }

  /**
   * 批量标记删除指定的`District`对象。
   *
   * @param {Array<string|number|bigint>} ids
   *     待批量删除的`District`对象的ID列表。
   * @param {boolean} showLoading
   *     是否显示加载提示。
  ...
  batchDelete(ids, showLoading = true) {
    return batchDeleteImpl(this, '/district/batch', ids, showLoading);
  }
  ...
  restore(id, showLoading = true) {
    return restoreImpl(this, '/district/{id}', id, showLoading);
  }

  /**
   * 根据代码，恢复一个被标记删除的`District`对象。
   *
   * @param {string} code
   *     要恢复的`District`对象的代码，该对象必须已经被标记删除。
   * @param {boolean} showLoading
   *     是否显示加载提示。
   * @return {Promise<void|ErrorInfo>}
   *     此HTTP请求的`Promise`对象。若操作成功，则解析成功且没有返回值；若操作失败，
  ...
  restoreByCode(code, showLoading = true) {
    return restoreByKeyImpl(this, '/district/code/{code}', 'code', code, showLoading);
  }

  /**
   * 批量恢复已被标记删除的`District`对象。
   *
   * @param {Array<string|number|bigint>} ids
   *     待批量恢复的已被标记删除的`District`对象的ID列表。
   * @param {boolean} showLoading
   *     是否显示加载提示。
   * @return {Promise<number|ErrorInfo>}
  ...
  batchRestore(ids, showLoading = true) {
    return batchRestoreImpl(this, '/district/batch', ids, showLoading);
  }
  ...
  purge(id, showLoading = true) {
    return purgeImpl(this, '/district/{id}/purge', id, showLoading);
  }

  /**
   * 根据代码，清除一个被标记删除的`District`对象。
   *
   * @param {string} code
   *     要清除的`District`对象的代码，该对象必须已经被标记删除。
   * @param {boolean} showLoading
   *     是否显示加载提示。
   * @return {Promise<void|ErrorInfo>}
   *     此HTTP请求的`Promise`对象。若操作成功，则解析成功且没有返回值；若操作失败，
  ...
  purgeByCode(code, showLoading = true) {
    return purgeByKeyImpl(this, '/district/code/{code}/purge', 'code', code, showLoading);
  }

  /**
   * 彻底清除全部已被标记删除的`District`对象。
   *
   * @param {boolean} showLoading
   *     是否显示加载提示。
   * @return {Promise<void|ErrorInfo>}
  ...
  purgeAll(showLoading = true) {
    return purgeAllImpl(this, '/district/purge', showLoading);
  }

  /**
   * 批量彻底清除已被标记删除的`District`对象。
   *
   * @param {Array<string|number|bigint>} ids
   *     待批量彻底清除的已被标记删除的`District`对象的ID列表。
   * @param {boolean} showLoading
   *     是否显示加载提示。
   * @return {Promise<number|ErrorInfo>}
  ...
  batchPurge(ids, showLoading = true) {
    return batchPurgeImpl(this, '/district/batch/purge', ids, showLoading);
  }

  /**
   * 彻底清除指定的`District`对象（无论其是否被标记删除）。
   *
   * @param {string|number|bigint} id
   *     要彻底清除的`District`对象的ID。
   * @param {boolean} showLoading
   *     是否显示加载提示。
   * @return {Promise<void|ErrorInfo>}
   *     此HTTP请求的`Promise`对象。若操作成功，则解析成功且没有返回值；若操作失败，
  ...
  erase(id, showLoading = true) {
    return eraseImpl(this, '/district/{id}/erase', id, showLoading);
  }

  /**
   * 根据代码，彻底清除指定的`District`对象（无论其是否被标记删除）。
   *
   * @param {string} code
   *     要彻底清除的`District`对象的代码。
   * @param {boolean} showLoading
   *     是否显示加载提示。
   * @return {Promise<void|ErrorInfo>}
   *     此HTTP请求的`Promise`对象。若操作成功，则解析成功且没有返回值；若操作失败，
  ...
  eraseByCode(code, showLoading = true) {
    return eraseByKeyImpl(this, '/district/code/{code}/erase', 'code', code, showLoading);
  }

  /**
   * 批量彻底清除指定的`District`对象（无论其是否被标记删除）。
   *
   * @param {Array<string|number|bigint>} ids
   *     待批量彻底清除的`District`对象的ID列表。
   * @param {boolean} showLoading
   *     是否显示加载提示。
   * @return {Promise<number|ErrorInfo>}
  ...
  batchErase(ids, showLoading = true) {
    return batchEraseImpl(this, '/district/batch/erase', ids, showLoading);
  }
  ...
  exportXml(criteria = {}, sortRequest = {}, autoDownload = true, showLoading = true) {
    return exportImpl(this, '/district/export/xml', 'XML', criteria, sortRequest, autoDownload, showLoading);
  }

  /**
   * 导出符合条件的`District`对象为JSON文件。
   *
   * @param {object} criteria
   *     查询条件参数，所有条件之间用`AND`连接。允许的条件包括：
   *  - `cityId: string|number|bigint` 所属城市的ID；
  ...
  exportJson(criteria = {}, sortRequest = {}, autoDownload = true, showLoading = true) {
    return exportImpl(this, '/district/export/json', 'JSON', criteria, sortRequest, autoDownload, showLoading);
  }

  /**
   * 导出符合条件的`District`对象为Excel文件。
   *
   * @param {object} criteria
   *     查询条件参数，所有条件之间用`AND`连接。允许的条件包括：
   *  - `cityId: string|number|bigint` 所属城市的ID；
  ...
  exportExcel(criteria = {}, sortRequest = {}, autoDownload = true, showLoading = true) {
    return exportImpl(this, '/district/export/excel', 'Excel', criteria, sortRequest, autoDownload, showLoading);
  }

  /**
   * 导出符合条件的`District`对象为CSV文件。
   *
   * @param {object} criteria
   *     查询条件参数，所有条件之间用`AND`连接。允许的条件包括：
   *  - `cityId: string|number|bigint` 所属城市的ID；
  ...
  exportCsv(criteria = {}, sortRequest = {}, autoDownload = true, showLoading = true) {
    return exportImpl(this, '/district/export/csv', 'CSV', criteria, sortRequest, autoDownload, showLoading);
  }

  /**
   * 从XML文件导入`District`对象。
   *
   * @param {File} file
   *     XML文件对象。
   ...
   *     是否显示加载提示。
   * @return {Promise<number|ErrorInfo>}
   *     此HTTP请求的`Promise`对象。若操作成功，则解析成功并返回成功导入的`District`对象的数量；
   *     若操作失败，则解析失败并返回一个`ErrorInfo`对象。
   */
  @Log
  importXml(file, parallel = false, threads = null, showLoading = true) {
    return importImpl(this, '/district/import/xml', 'XML', file, parallel, threads, showLoading);
  }

  /**
   * 从JSON文件导入`District`对象。
   *
   * @param {File} file
   *     JSON文件对象。
   * @param {boolean} parallel
  ...
  importJson(file, parallel = false, threads = null, showLoading = true) {
    return importImpl(this, '/district/import/json', 'JSON', file, parallel, threads, showLoading);
  }

  /**
   * 从Excel文件导入`District`对象。
   *
   * @param {File} file
   *     Excel文件对象。
   * @param {boolean} parallel
  ...
  importExcel(file, parallel = false, threads = null, showLoading = true) {
    return importImpl(this, '/district/import/excel', 'Excel', file, parallel, threads, showLoading);
  }

  /**
   * 从CSV文件导入`District`对象。
   *
   * @param {File} file
   *     CSV文件对象。
   * @param {boolean} parallel
  ...
  importCsv(file, parallel = false, threads = null, showLoading = true) {
    return importImpl(this, '/district/import/csv', 'CSV', file, parallel, threads, showLoading);
  }
}
